...
for line in fileinput.input([snpfile]):#iterates over the lines of all files listed in sys.argv[1:],
		...
		info = trim(info1)
                print info
                print info[int(strain)]
		if info[int(strain)] != "." and info[int(strain)].startswith("1/1:1"):
			if "," not in info[4]:
